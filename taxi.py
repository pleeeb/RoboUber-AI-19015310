...
class Taxi:
      ...
      def recvMsg(self, msg, **args):
          ...
          # we just dropped off a fare and received payment, add it to the account
             ...
             self._totalPayments += args["amount"]
             self._numPayments += 1
             return
      ...
      def _planPath(self, origin, destination, **args):
          ...
          # Visited nodes
          closedlist = {}
          # Distance of origin to node
          origindist={}
          origindist[origin] = 0
          # Keep a dict of node - parent values for tracking end route
          parent = {}
          parent[origin] = origin
          while openlist:
              # Get node with best heuristic
              point = self.removeShortest(openlist)
              # If node is destination node then reconstruct the path to origin node and return the value
              if point == destination:
                  reconst_path = []
                  while parent[point] != point:
                      reconst_path.append(point)
                      point = parent[point]
                  reconst_path.append(origin)
                  reconst_path.reverse()
                  return reconst_path
              # Remove node with best heuristic
              openlist.pop(point)
              # Iterate through all neighbours of the node
              for node in self._map[point].keys():
                  # If not seen neighbour then add to openlist with travel time to destination, record parent node
                  # and distance from origin.
                  if node not in openlist and node not in closedlist:
                      openlist[node] = (self._world.travelTime(self._world.getNode(
                                        node[0],node[1]), self._world.getNode(destination[0],destination[1])))
                      parent[node] = point
                      origindist[node] = origindist[point]+(self._world.travelTime(self._world.getNode(
                                        node[0],node[1]), self._world.getNode(destination[0],destination[1])))
                  else:
                      # If current distance to neighbour is more than the distance to current point + travel time to neighbour
                      # then update the value with the new lower distance, set the parent node to the current point
                      if origindist[node] > origindist[point] +(self._world.travelTime(self._world.getNode(
                      node[0],node[1]), self._world.getNode(point[0],point[1]))):
                          origindist[node] = origindist[point]+(self._world.travelTime(self._world.getNode(
                      node[0],node[1]), self._world.getNode(point[0],point[1])))
                          parent[node] = point
                          # If neighbour was in closedlist, remove from list and add to openlist
                          if node in closedlist:
                              closedlist.pop(node)
                              openlist[node] = (self._world.travelTime(self._world.getNode(
                      node[0],node[1]), self._world.getNode(destination[0],destination[1])))
              closedlist[point] = 0
          # Return empty list if no path to destination is found
          return []
      ...
      def removeShortest(self,dict):
          # Return point with shortest cost
          min=float("inf")
          point = ()
